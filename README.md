...
* The robot will be setup on a tablet computer in-store and can be downloaded as a native application on a user's smartphone through scanning a QR code at a point-of-sales machine (i.e. cash register). The application will contain an HTML5 form that contains the following input fields:
	* A text field for the customer's first and last name
	* A email text field for a customer's email
	* A url text field for a customer's personal URL (i.e. LinkedIn URL for organization recruitment purposes)
